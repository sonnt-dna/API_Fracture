from catboost_ai.optuna_catboost import Train_CAT
from catboost_ai.score_cal import Score
...
# set scoring
scoring='f1_weighted'
model = Train_CAT(features=X_train,
               labels=y_train,
               iterations=20, # number of iterations
               scoring=scoring,
               validation_size=0.1,
               objectives=0, #{0: "valid_score", 1: "train_test_drop"}
               favor_class=1, #use when scoring is 'f1_weighted': {0: "min false negative", 1: "min false positive", 2: "balanced"}
               show_shap=True, # flag to show shap True or False
               )

y_pred=model.predict(cat.Pool(data=X_test, label=y_test)) >= 0.5
