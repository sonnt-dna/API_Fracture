...
def OptunaFinder(features: any = None,
                 labels: any = None,
    ...
    for i, name in enumerate(models.keys()):
        ...
        clf, train_score, valid_score = Optuna_Classifier(
            features=features,
            labels=labels,
            algorithm=name,
    ...
    try:
        for inx in scores.index:
            if scores.loc[inx, "Validation"] >= base_score and scores.loc[
                inx, "Drop (Train-Valid)"] <= max_train_valid_drop:
